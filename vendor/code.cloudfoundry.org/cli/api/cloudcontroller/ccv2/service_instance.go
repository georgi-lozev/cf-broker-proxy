...
type serviceInstanceRequestBody struct {
...
func (client *Client) CreateServiceInstance(name string, servicePlanGUID string, spaceGUID string, acceptsIncomplete bool, parameters map[string]interface{}) (ServiceInstance, Warnings, error) {
	requestBody := serviceInstanceRequestBody{
	...
	return serviceInstance, response.Warnings, nil
}

func (client *Client) DeleteServiceInstance(serviceInstanceGUID string, acceptsIncomplete bool) (ServiceInstance, Warnings, error) {
	request, err := client.newHTTPRequest(requestOptions{
		RequestName: internal.DeleteServiceInstanceRequest,
		URIParams:   map[string]string{"service_instance_guid": serviceInstanceGUID},
		Query:       url.Values{"accepts_incomplete": {strconv.FormatBool(acceptsIncomplete)}, "recursive": {"true"}},
	})
	if err != nil {
		return ServiceInstance{}, nil, err
	}
